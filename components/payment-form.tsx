...
export function PaymentForm({ payment, subscription, onCancel, onSave }: { payment: Payment; subscription: Subscription; onCancel: () => void; onSave: (value: PaymentEditInput) => void }) {
  const [value, setValue] = useState<PaymentEditInput>({ paymentDate: payment.paymentDate, amount: payment.amount, status: payment.status === "estimated" ? "paid" : payment.status, note: payment.note ?? "" });
  ...
  return <form className="price-change-form" onSubmit={(event) => { event.preventDefault(); if (valid) onSave(value); }}>
    <div className="price-change-note"><strong>Correct this past charge</strong><p>Saving an estimated row turns it into a recorded payment. If this is the newest paid charge, its amount becomes the renewal cost used in monthly and yearly calculations.</p></div>
    ...
    <div className="modal-actions"><button type="button" className="button secondary" onClick={onCancel}>Cancel</button><button className="button primary" disabled={!valid}>Save payment</button></div>
  </form>;
}

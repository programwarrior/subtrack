...
export function SubscriptionDetails({ subscription, settings, onEdit, onPriceChange, onUpdate, onPaid, onDelete }: { subscription: Subscription; settings: Settings; onEdit: () => void; onPriceChange: () => void; onUpdate: (patch: Partial<Subscription>) => void; onPaid: () => void; onDelete: () => void }) {
  ...
  return <div className="detail-body">
    <div className="detail-hero">
      ...
      <div><span className={`status-pill ${subscription.status}`}>{subscription.status}</span><p className="detail-price">{formatMoney(subscription.price, subscription.currency)} <span>/ {frequencyLabel(subscription.billingFrequency).toLowerCase()}</span></p></div>
    </div>
    ...
    <div className="detail-stats"><div><span>Monthly equivalent</span><strong>{formatMoney(monthlyEquivalent(subscription), settings.currency)}</strong></div><div><span>Yearly estimate</span><strong>{formatMoney(yearlyEquivalent(subscription), settings.currency)}</strong></div></div>

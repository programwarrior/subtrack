...
type ModalState = "add" | "add-import" | "edit" | "details" | "payment-add" | "payment-edit" | "price-change" | "settings" | "smart-import" | null;
...
export function Dashboard() {
  ...
  return <>
    ...
    <Modal open={modal === "details" && Boolean(selected)} onClose={() => setModal(null)} title={selected?.name ?? "Subscription"} eyebrow="Subscription details">{selected && <SubscriptionDetails subscription={selected} settings={store.settings} onEdit={() => setModal("edit")} onAddPayment={() => setModal("payment-add")} onEditPayment={(paymentId) => { setSelectedPaymentId(paymentId); setModal("payment-edit"); }} onPriceChange={() => setModal("price-change")} onUpdate={(patch) => { store.updateSubscription(selected.id, patch); setToast("Subscription updated."); }} onPaid={paid} onDelete={() => { store.deleteSubscription(selected.id); setModal(null); setToast("Subscription deleted."); }} />}</Modal>
    <Modal open={modal === "payment-add" && Boolean(selected)} onClose={() => setModal("details")} title="Add missing charge" eyebrow={selected?.name}>{selected && <PaymentForm subscription={selected} onCancel={() => setModal("details")} onSave={(value) => { store.addPayment(selected.id, value); setModal("details"); setToast("Missing charge added and totals recalculated."); }} />}</Modal>

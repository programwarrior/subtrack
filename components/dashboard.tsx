...
import { daysUntil, formatDate, formatMoney, frequencyLabel, monthlyEquivalent, yearlyEquivalent } from "@/lib/calculations";
...
type ModalState = "add" | "edit" | "details" | "price-change" | "settings" | "smart-import" | null;
...
export function Dashboard() {
  ...
  return <>
      ...
      <main id="main">
        {store.subscriptions.length === 0 ? <section className="empty-state"><EmptyIcon /><p className="eyebrow">Everything in one calm place</p><h1>Your subscriptions will appear here.</h1><p>Add your first subscription or scan existing files to see upcoming payments and monthly spending.</p><div><button className="button primary" onClick={() => setModal("add")}><Plus size={18} /> Add subscription</button><button className="button secondary" onClick={() => setModal("smart-import")}><ScanLine size={17} /> Smart import</button><button className="button quiet" onClick={() => { store.loadDemo(); setToast("Demo data loaded."); }}><Sparkles size={17} /> Load demo data</button></div></section> : <>
          ...
          <section className="summary-card" aria-label="Spending summary">
            ...
            <div className="summary-metric next-metric"><p>Next payment</p>{upcoming[0] ? <><strong>{upcoming[0].name}</strong><small>{formatMoney(upcoming[0].price, upcoming[0].currency)} · {formatDate(upcoming[0].nextPaymentDate, store.settings.dateFormat, true)}</small></> : <><strong>Nothing due</strong><small>You’re all clear</small></>}</div>
          </section>
    ...
    <Modal open={modal === "smart-import"} onClose={() => setModal(null)} title="Smart import" eyebrow="Add subscriptions from your files" wide><SmartImport currency={store.settings.currency} existingNames={store.subscriptions.map((item) => item.name)} onClose={() => setModal(null)} onImport={(items) => { items.forEach((item) => store.addSubscription(item)); setModal(null); setToast(`${items.length} subscription${items.length === 1 ? "" : "s"} imported.`); }} /></Modal>
